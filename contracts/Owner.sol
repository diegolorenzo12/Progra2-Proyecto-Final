...
interface OwnersInterface{
    ...
    function getOwnerRole(address _owner) external view returns(uint);
}
...
contract Owners {
    ...
    function addOwner (string memory _name, string memory _description, OwnerRole _role) public{
        owners[msg.sender] = Owner(_name, _description, _role, true);
    }

    function getOwner(address owners_addr) public view returns (string memory, string memory, OwnerRole){
        return (owners[owners_addr].name, owners[owners_addr].description, owners[owners_addr].role);
    }
